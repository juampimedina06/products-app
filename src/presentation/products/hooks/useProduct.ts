...
export const useProduct = (productId: string) => {
    ...
    const productQuery = useQuery({
        ...
        queryFn: () => getProductById(productId), //funcion que nos va a traer la data
        staleTime: 1000 * 60 * 60 //1 hora  //esto ayuda que se guarde en cache

    })

    //Mutacion (manualment se tiene que hacer llamar no funciona como el productQuery que se dispara apenas entra)
